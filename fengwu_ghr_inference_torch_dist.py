...
class FengWu_GHR_Inference:
    ...
    def inference(self, 
        ...
        write_grib(datasample_initial, 
                   ...
                   filter_dict = self.cfg.save_cfg.variables_list,
                    region = self.cfg.save_cfg.region)
        ...
        for step in range(self.cfg.inference_steps):   
            ...
            et1 = time.time()            
            logger.info(f'Step: {step}, Initial T: {in_time_stamp}, forecast T: {timestamp}, inference speed: {(et1-st):.2f}s')
            ...
            write_grib(datasample, 
                    ...
                    region = self.cfg.save_cfg.region)
            et2 = time.time()        
            logger.info(f' forecasts are saved at {self.output_root} with nc format , save time is: {(et2-et1):.2f}s')
    ...
    def read_analysis_initial_field(self, timestamp):
        ...
        try:
            ...
            logger.info("An error occurred with the second file:", e)
...
def parse_args():
    ...
    parser.add_argument(
        '--slurm',
        action='store_true',
        help='Enable SLURM support. Use SLURM environment variables to assign GPUs.'
    )
    parser.add_argument('--rank', type=int, default=0)
    parser.add_argument('--world_size', type=int, default=1)
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--local_rank', type=int, default=0)
    parser.add_argument('--port', type=int, default=23135) 
...
def inference_task(local_rank, cfg, timestamps):
    """
    Perform inference on a specific GPU for a list of timestamps.
    """
    # torch.cuda.set_device(local_rank)  # Set the current GPU device
    FengWu_GHR = FengWu_GHR_Inference(cfg=cfg)

    for timestamp in timestamps:
        logger.info(f"GPU {local_rank} processing timestamp: {timestamp}")
        FengWu_GHR.inference(timestamp)
    logger.info(f"GPU {local_rank} completed all tasks!")
...
def main():
    ...
    logger.warning(f"Process {args.rank}/{args.world_size} on GPU {args.local_rank} starting inference.")

    # Parse timestamps
    timestamps = parse_timestamp(args.timestamp)
    if len(timestamps) == 2:  # Range of timestamps
        timestamps = pd.date_range(start=timestamps[0], end=timestamps[1], freq='6H')
        timestamps = timestamps.strftime("%Y-%m-%d %H:%M:%S").tolist()
    

    # Split timestamps among GPUs
    timestamps_per_gpu = len(timestamps) // args.world_size
    remainder = len(timestamps) % args.world_size
    start_idx = args.rank * timestamps_per_gpu + min(args.rank, remainder)
    end_idx = start_idx + timestamps_per_gpu + (1 if args.rank < remainder else 0)
    timestamps_assigned = timestamps[start_idx:end_idx]

    # Perform inference
    inference_task(args.local_rank, cfg, timestamps_assigned)
    ...
    logger.info(f"Process {args.rank} on GPU {args.local_rank} finished.")
